...
<body>
	...
	<h1>Type</h1>
	숫자: <fmt:formatNumber value="${num }" type="number"/>	<%-- number가 default --%>
	<br>
	...
	통화(달러) : <fmt:formatNumber value="${num }" type="currency" currencySymbol="$"/><br>
	
	<h1>var로 변수에 저장</h1>
	<fmt:formatNumber var="num$" value="${num }" type="currency" currencySymbol="$"/><br>
	...
	<h1>Date 객체를 String 형태로 출력 > fmt:fomatDate</h1>
	${today } <br>
	...
	<h1>String을 Date 객체로 전환 > fmt:parseDate</h1>
	<fmt:parseDate var="date2" value="${pattern2}" pattern="yyyy/MM/dd/ HH:mm:ss"/><br>
	pattern2 : ${pattern2 }<br>
	date2 : ${date2 }
